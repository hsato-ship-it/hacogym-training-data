// ================================
// ハコジム トレーニング ロジック（イベント駆動版 最終調整 v2）
// ================================
(async () => {
  const DEBUG = true;
  ...
  if (resultSectionEl) resultSectionEl.style.display = "none";
  ...
  // flow:end は「一連の音声がすべて終わった」= エンドカード音声後
  window.addEventListener("flow:end", () => {
    log("🏁 flow:end");
    ui.generateResults(); // ここで成果カード生成＆スクロール
  });
  ...
  trainFlowAudios.forEach((a, i) => {
    ...
    a.addEventListener("play", () => {
      ...
      // ✅ 終了カードがアクティブになったらホバーボタンを「成果カードを表示」に変更
      const card = a.closest(".card");
      if (card?.classList.contains("end-card")) {
  ...
  document.getElementById("startBtn").addEventListener("click", async () => {
    ...
    document.getElementById("endSessionBtn").addEventListener("click", () => {
      if (confirm("本当に終了しますか？")) {
        ...
        // ここで直接成果カードを出す（スクロールは generateResults 内）
        ui.generateResults();
      }
    });
  });
  ...
  document.getElementById("copyResultBtn").addEventListener("click", async () => {
    ...
    setTimeout(() => (b.textContent = "成果をコピー"), 1500);
  });

  // -9- 成果生成（未入力は「（記録なし）」表記／表示後はメニューだけ） ---
  ...
  ui.generateResults = function () {
    ...
    cards.forEach((card) => {
      ...
      rows.forEach((r) => {
        ...
        const wRaw = r.querySelector(".w-input")?.value ?? "";
        const rRaw = r.querySelector(".r-input")?.value ?? "";
        const weight = String(wRaw).trim();
        const reps = String(rRaw).trim();
  ...
  ui.showVersion("training_logic.js v2025-10-23-final-v2");
})();
